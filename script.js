...
// Project gallery — page pagination (haptic page-select circles)
(function () {
  ...
  const showPage = pageNum => {
    pages.forEach(page => {
      const match = page.dataset.page === pageNum;
      page.hidden = !match;
      page.classList.toggle('is-active', match);
    });
    dots.forEach(dot => {
      const active = dot.dataset.page === pageNum;
  ...
  dots.forEach(dot => {
    dot.addEventListener('click', () => {
      buzz();
      showPage(dot.dataset.page);
      pagination.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
    });
    // Haptic press feedback, matching the header nav pattern
    dot.addEventListener('touchstart', () => { dot.classList.add('pressed'); }, { passive: true });
    dot.addEventListener('touchend', () => dot.classList.remove('pressed'));
    dot.addEventListener('touchcancel', () => dot.classList.remove('pressed'));
    dot.addEventListener('mousedown', () => dot.classList.add('pressed'));
    dot.addEventListener('mouseup', () => dot.classList.remove('pressed'));
    dot.addEventListener('mouseleave', () => dot.classList.remove('pressed'));
  });
})();
